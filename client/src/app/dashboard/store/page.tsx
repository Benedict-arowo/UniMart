...
export default function StorePage() {
  ...
  const handleFileUpload = (field: 'banner' | 'logo') => (file: File) => {
    setStoreSettings(prev => ({ ...prev, [field]: URL.createObjectURL(file) }))
  }
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input id="name" name="name" value={storeSettings.name} onChange={handleSettingsChange} />
          </div>
          <div>
            <label htmlFor="description" className="block text-sm font-medium text-gray-700">Store Description</label>
            <Textarea id="description" name="description" value={storeSettings.description} onChange={handleSettingsChange} />
          </div>
          <div>
            <label htmlFor="banner" className="block text-sm font-medium text-gray-700">Banner Image</label>
            <FileUploader id="banner" accept="image/*" onFileSelect={handleFileUpload('banner')} />
            {storeSettings.banner && <img src={storeSettings.banner} alt="Store Banner" className="mt-2 h-40 w-full object-cover" />}
          </div>
          <div>
            <label htmlFor="logo" className="block text-sm font-medium text-gray-700">Logo</label>
            <FileUploader id="logo" accept="image/*" onFileSelect={handleFileUpload('logo')} />
            {storeSettings.logo && <img src={storeSettings.logo} alt="Store Logo" className="mt-2 h-20 w-20 object-contain" />}
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="border rounded-lg p-4 space-y-4">
            <header className="flex items-center justify-between">
              <img src={storeSettings.logo} alt="Store Logo" className="h-12 w-12 object-contain" />
              <h2 className="text-2xl font-bold">{storeSettings.name}</h2>
            </header>
            <img src={storeSettings.banner} alt="Store Banner" className="w-full h-40 object-cover" />
